...
public class Skills {
	...
	public Skills(final DreamBlaster ctx) {
		this.ctx = ctx;
	}
	...
	public int getCurrentExp(final int index) {
		...
		return ctx.getSkills().getExperience(Skill.forId(index));
	}
	...
	public int getCurrentLevel(final int index) {
		...
		return ctx.getSkills().getDynamic(Skill.forId(index));
	}
	...
	public int getRealLevel(final int index) {
		...
		return ctx.getSkills().getStatic(Skill.forId(index));
	}
	...
	public long getTimeTillLevel(final int index, final int exp,
		...
		try {
			return time * getExpToLevel(index, endLvl) / (exp);
	...
	public int ammountTillLevel(final int index, final double exp, final int lvl) {
		...
		return getExpToLevel(index, lvl) != -1 ? (int) (getExpToLevel(index,
				lvl) / exp) : 0;
	}
	 ...
	 */
	public boolean doHover(final Skill s) {
		// methods.game.openTab(Game.Tab.STATS);
		if (ctx.getTabs().getOpen() != Tab.SKILLS) {
			ctx.getTabs().open(Tab.SKILLS);
		}
		try {
			MethodProvider.sleep(MethodProvider.random(10, 100));
			return ctx.getSkills().hoverSkill(s);
